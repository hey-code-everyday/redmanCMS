<template>
  <div class="hero-wide-search">
   <div class='hero__social-icons' :class="isAdvancedSearch ? 'advanced-search-social-icons' : 'default-social-icons' ">
      <div class='hero__icons-wrapper'>
        <social-icons :vertical="true" :size="24" />
      </div>
    </div>
    <div class="hero strap-container container pad-medium">
      <div class='hero-head' v-if='strap_editing || (strap_state.agentImage.imageUrl || strap_state.logoImage.imageUrl)'>
...
export default {
  ...
  computed: {
    isAdvancedSearch() {
      return this.strap_state.footerComponent === 'advancedSearch' ? true : false
    }
  },
  ...
  initialState (strap, state) {
    return {
      text: defaultQuill(),
      title: defaultTitle({ html: 'Hero Wide Search' }),
