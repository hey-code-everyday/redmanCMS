    ...
    <div class="card-image">
      <template v-if="listing.images && listing.images.length">
        ...
        <figure :class=" { hover: isActive }" class="image" :style="`background-image: url(${imageSrc})`">
          <img  :alt="getAlt()" :src="imageSrc" :srcset="imageSrcset" />
        </figure>
      </template>
      <template v-else>
      ...
      <div class="overlay is-fg" :class=" { hover: isActive }">
        <div class="overlay-inner">
          ...
          <!-- <h4 v-if="listing.bed ||">{{ listing.price }}</h4> -->
        </div>
      </div>
    </div>
  </div>
...
import { encodeDetailsUrl } from '@/helpers/listings'
import favoriteListingMixin from '@/mixins/favorite-listing'

export default {
  name: 'largeListingCard',
  ...
  methods: {
    ...
    getAlt () {
      if (this.listing.address && this.listing.id) return "MLS® listing #"+ this.listing.id + " for sale located at " + this.listing.address
      if (this.listing.id) return "MLS® listing #"+ this.listing.id + " for sale"
      return ''
    },
