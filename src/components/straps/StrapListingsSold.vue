    ...
    <div class="strap-container container pad-small">
      ...
      <div class="strap-content">
              ...
              <div class="level-item">
                ...
                >
                  <div class="dropdown-trigger">
                    <span @click="toolbar.sortOpen = !toolbar.sortOpen">
                      <span
                        >Sort by
                        <strong>{{
                          toolbar.sortOptions[toolbar.filters.sort]
                        }}</strong></span
                      >
                      <span class="icon is-small">
                        <i class="icon-chevron-down"></i>
                  ...
                  <div class="dropdown-menu">
                    <div class="dropdown-content">
                      <a
                        v-for="(val, key) in toolbar.sortOptions"
                        class="dropdown-item"
                        @click="sortBy(key)"
                        >{{ val }}</a
                      >
        ...
        <template v-else-if="loading">
          <div
            class="strap-listings__loading pad"
            :style="{color: strap_palette.text}"
          >
            <span
              class="loader"
              :style="{
                borderLeftColor: strap_palette.text,
                borderBottomColor: strap_palette.text
              }"
            ></span
            ><br />
            Loading...
        ...
        <div class="strap-buttons" v-if="!moreResults">
          <a
            href="/search"
            class="button strap-button is-primary"
            @click="fetchNext"
          >
        ...
        <div class="strap-buttons" v-else-if="!error">
          ...
          <div v-else class="button strap-button is-primary is-loading"></div>
...
import ListingCardSold from '@/components/ListingCardSold'
...
import {prefetchSoldListings, preconfiguredOptions} from '@/helpers/listings'

let Velocity = null
if (process.browser) Velocity = require('velocity-animate')

export default {
  name: 'StrapListingsSold',
  ...
  prefetch(resolve, reject, store, state) {
    prefetchSoldListings(resolve, reject, store, state, {size: 9})
  },
  ...
  afterPrefetch(err, result) {
    ...
    const size = this.isMobile ? 1 : this.isTablet ? 4 : 3
  ...
  methods: {
    fetchData(cached) {
      ...
      filters.size = this.isMobile ? 1 : this.isTablet ? 4 : 3
      filters.from = 0
    ...
    beforeEnter: function(el) {
      ...
      Velocity(el, {opacity: 0, translateY: '30px'})
    },
        ...
        tabs: [
          {
            ...
            fields: [
              ...
              {
                key: 'state.defaultListingAmount',
                type: 'number',
                label: 'Input Default Number Of Listings',
                default: 9,
                hint: 'Number of listings to display. Between 3 and 50.',
                minValue: 3,
                maxValue: 50,
                step: 1
              }
