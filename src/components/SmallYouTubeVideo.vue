<template>
  <div :style="videoStyle" :class="[ 'youtube-video', highres && 'is-high-res' ]">
  ...
  methods: {
    fetchVideoData () {
      ...
      if (highres) {
        ...
        img.onload = () => {
          if (img.width <= 120) {
            this.$set(this.video, 'thumbnail', `https://img.youtube.com/vi/${videoID}/hqdefault.jpg`)
          }
        }
        img.onerror = () => {
          this.$set(this.video, 'thumbnail', `https://img.youtube.com/vi/${videoID}/hqdefault.jpg`)
...
.play-button
  ...
  +touch
    ...
    svg
      transform: translate(-50%, -50%) rotate(90deg) scale(0.25) !important
